...
public abstract class BaseSimpleItemTypeRecyclerAdapter<ItemBean> extends RecyclerView.Adapter<BaseRecyclerViewHolder> implements IHeaderFootView {
    ...
    static final int VIEW_TYPE_NORMAL = Integer.MAX_VALUE-Integer.MIN_VALUE;
    static final int VIEW_TYPE_FOOT = Integer.MAX_VALUE;

    public BaseSimpleItemTypeRecyclerAdapter() {
    ...
    public abstract void bindData(BaseRecyclerViewHolder holder, ItemBean itemBean, int position);

    @Override
    public BaseRecyclerViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        if (viewType == VIEW_TYPE_HEADER&&mHeaderViews.size()>0) {
            return new NonGeneralViewHolder(mHeaderViews.get(0));
        } else if (viewType == VIEW_TYPE_FOOT&&mFootViews.size()>0) {
            return new NonGeneralViewHolder(mFootViews.get(0));
        } else {
    ...
    public void onBindViewHolder(final BaseRecyclerViewHolder holder, final int position) {
        ...
        if (realPosition < adapterCount) {
            bindData(holder, mDatas.get(realPosition), realPosition);
    ...
    private static class NonGeneralViewHolder extends BaseRecyclerViewHolder {

        public NonGeneralViewHolder(View itemView) {
            super(itemView);
        }
    }
}
